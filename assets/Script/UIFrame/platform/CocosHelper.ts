...
export default class CocosHelper {
   ...
   * @param tweens
   */
  public static async runRepeatTweenSync(
    target: any,
    repeat: number,
    ...tweens: cc.Tween[]
  ) {
  ...
  /** 同步的tween */
  public static async runTweenSync(
    target: any,
    ...tweens: cc.Tween[]
  ): Promise<void> {
    return new Promise((resolve, reject) => {
  ...
  public static async runActionSync(
    node: cc.Node,
    ...actions: cc.FiniteTimeAction[]
  ) {
  ...
  /** 加载资源异常时抛出错误 */
  public static loadResThrowErrorSync<T>(
    url: string,
   ...
   */
  private static _progressCallback(
    completedCount: number,
    totalCount: number,
    item: any
  ) {
  ...
  /** 路径是相对分包文件夹路径的相对路径 */
  public static loadAssetFromBundleSync(
    bundleName: string,
    url: string | string[]
  ) {
    ...
    return new Promise((resolve, reject) => {
      bundle.load(url, (err, asset: cc.Asset | cc.Asset[]) => {
        if (err) {
          cc.error(
            `加载bundle中的资源失败, 未找到asset, url:${url}, err:${err}`
          );
          resolve(null);
        } else {
          resolve(asset);

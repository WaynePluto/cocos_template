 ...
 * @ Modified by: liuwei
 * @ Modified time: 2020-10-08 15:14:48
...
export default class UIManager extends cc.Component {
  ...
  /**正在加载的form */
  private _LoadingForm: { [key: string]: boolean } = cc.js.createMap();
   ...
   * (代替之前的预制体挂载)
   */
  private static createUIRootNode() {
    let canvas = cc.find("Canvas");
    let sceneNode = new cc.Node("Scene");
    sceneNode.setParent(canvas);
    let uiRootNode = new cc.Node("UIROOT");
    uiRootNode.setParent(cc.find("Canvas/Scene"));
    uiRootNode.addChild(new cc.Node("Screen"));
    uiRootNode.addChild(new cc.Node("FixedUI"));
    uiRootNode.addChild(new cc.Node("PopUp"));
    uiRootNode.addChild(new cc.Node("TopTips"));
    uiRootNode.addChild(new cc.Node("UIModalManager"));
    uiRootNode.addChild(new cc.Node("UIAdapterManager"));
    let inputBlocker = new cc.Node("InputBlocker");
    inputBlocker.addComponent(cc.BlockInputEvents);
    inputBlocker.addComponent(cc.Widget);
    let comWidget = inputBlocker.getComponent(cc.Widget);
    comWidget.target = canvas;
    comWidget.top = 0;
    comWidget.bottom = 0;
    comWidget.left = 0;
    comWidget.right = 0;
    this.instance = uiRootNode.addComponent(this);
    cc.director.once(cc.Director.EVENT_AFTER_SCENE_LAUNCH, () => {
      this.instance = null;
    });
  }
  ...
  public async openUIForm(prefabPath: string, ...params: any) {
    ...
    if (
      this.checkUIFormIsShowing(prefabPath) ||
      this.checkUIFormIsLoading(prefabPath)
    ) {
  ...
  private async clearStackArray() {
    if (
      ...
      this._StaCurrentUIForms.length <= 0
    ) {
      return;
    }
  ...
  public closeTopStackUIForm() {
    if (
      this._StaCurrentUIForms != null &&
      this._StaCurrentUIForms.length >= 1
    ) {
  ...
  private async pushUIFormToStack(prefabPath: string, ...params: any) {
    if (this._StaCurrentUIForms.length > 0) {
      let topUIForm = this._StaCurrentUIForms[
        this._StaCurrentUIForms.length - 1
      ];
    }
